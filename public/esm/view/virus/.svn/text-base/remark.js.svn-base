define(function(require){
    ...
    var op = {
		init:function(container,paramStr){
				...
				$(container).find('#tbClient').bootstrapTable({
                    ...
                    columns: [{
    					...
    					formatter: function(value,row) {
                            return  '<input value="' + (value?value:'') +'" id="' + row.sguid + '"/>';
                        }
                    }],
                    cache: false,
                    pageSize:4,
                    ...
                    onLoadSuccess:function(){
                        $(container).find('#tbClient').resizableColumns();
    					...
    					var $editText =  $(container).find('.remark-wrap td>input:text'),
    						...
    						function save(){
    							if(val != reRemarkVal){
    								//预留接口
    								RsCore.ajax('ep/setmemo', { sguid: id  ,memo:val}, function(data,r) {
                                        if(!r){
                    ...
                    onColumnSwitch:function(){
                        $(container).find('#tbClient').trigger('refresh'); 
                    }
                });
            ...
            objTable.find('.fixed-table-container').height(table_height-custom_height-table_fotter_height);
        },
    };
    return {
