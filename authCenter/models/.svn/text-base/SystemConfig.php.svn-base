...
class SystemConfigModel
{
    ...
    {
        return Mysql::getRow("SELECT passwd,salt FROM auth_manager WHERE userName = '$uName'");
    }
    public function modifyPassWd($uName, $newpwd1)
    {
        ...
        $mixid   = Mysql::exec("UPDATE auth_manager SET passwd='$pwdMD5',salt='$salt',modifiedTime='$mixTime' WHERE userName = '$uName'");
        if ($mixid > 0) {
            return true;
        } else {
            return ['msg' => '修改密码失败'];
    ...
    public function getCompanyLogo()
    {
        ...
        if (!empty($result)) {
            ...
            $logoPath= __dir__.'/../../public/auth/uploads/logon.png';
    ...
    public function modifySystemConfig($title, $subTitle)
    {
        $mixTime = date('Y-m-d H:i:s');
        $mixid   = Mysql::exec("UPDATE auth_base_info SET title='$title',subTitle='$subTitle',modifiedTime='$mixTime'");
        if ($mixid >= 0) {
            return true;
        } else {
            return ['msg' => '系统设置修改失败'];
