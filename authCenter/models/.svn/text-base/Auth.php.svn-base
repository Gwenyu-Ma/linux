...
class AuthModel
{
     ...
     * @return bool/Array
     */
    public static function importAuth($sn,$cb)
    {
        $auth=new Auth($sn);
        ...
        if($count<=0){
            $rCount=Mysql::exec('insert into auth_base_info (title,subTitle,addTime,modifiedTime) values(?,?,?,?)',[$authResult['mt'],$authResult['title'],date('Y-m-d H:m:s',time()),date('Y-m-d H:m:s',time())]);
            if($rCount<=0){
                return [msg=>'导入授权失败'];
            }
        }
        ...
        foreach($authResult['items'] as $item){
            if(!empty($item['context'])){
                Mysql::exec('insert into auth_grant(serialNo,accreditXML,ImportTime,notBefore,notAfter,descript,baseSN) values(?,?,?,?,?,?,?)',array_merge([
                    $item['sn'],
        ...
        foreach($authDict as $key=>$items){
            foreach ($items as $item) {
                Mysql::exec('insert into auth_product_list (seriaNO,productGuid,accreditCount,startTime,endTime,addTime,modifiedTime) values(?,?,?,?,?,?,?)',[
                    $key,
                    $item['pguid'],
                    $item['count'],
        ...
        foreach ($authProdcuts as $pguid => $value) {
            Redis::hSet('hau',$pguid,json_encode([
                'count'=>$value['count'],
                'os'=>1,
